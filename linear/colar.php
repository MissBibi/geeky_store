...
$maxRows_colar = 3;
$pageNum_colar = 0;
if (isset($_GET['pageNum_colar'])) {
  $pageNum_colar = $_GET['pageNum_colar'];
}
$startRow_colar = $pageNum_colar * $maxRows_colar;

mysql_select_db($database_geeky_store, $geeky_store);
$query_colar = "SELECT * FROM produto WHERE produto.tipo_codtipo=2 AND produto.status='Disponivel'";
$query_limit_colar = sprintf("%s LIMIT %d, %d", $query_colar, $startRow_colar, $maxRows_colar);
$colar = mysql_query($query_limit_colar, $geeky_store) or die(mysql_error());
$row_colar = mysql_fetch_assoc($colar);

if (isset($_GET['totalRows_colar'])) {
  $totalRows_colar = $_GET['totalRows_colar'];
} else {
  $all_colar = mysql_query($query_colar);
  $totalRows_colar = mysql_num_rows($all_colar);
}
$totalPages_colar = ceil($totalRows_colar/$maxRows_colar)-1;
...
$queryString_colar = sprintf("&totalRows_colar=%d%s", $totalRows_colar, $queryString_colar);
...
<html>
	...
	<!-- Featured -->
        ...
        <tbody>
          <?php do { ?>
          <tr>
           <td><?php echo $row_colar['codproduto']; ?></td>
              <td><?php echo $row_colar['nomep']; ?></td>
              <td><?php echo $row_colar['precou']; ?></td>
              <td><?php echo $row_colar['stock']; ?></td>
              <td><?php echo $row_colar['descricao']; ?></td>
              <td><input type="image" name="imageField" id="imageField" src="../imagens/<?php echo $row_colar['imagem']; ?>" width="200" height="160"></td>
              <td><a href="compras.php?codigo=<?php echo $row_colar['codproduto']; ?>"><img src="images/kart.png" width="200" height="160"></a></td>
            </tr>
            <?php } while ($row_colar = mysql_fetch_assoc($colar)); ?>
          
        </table>
			<p>&nbsp;
            <table border="0">
              <tr>
                <td><?php if ($pageNum_colar > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_colar=%d%s", $currentPage, 0, $queryString_colar); ?>"><img src="../imagens/First.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_colar > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_colar=%d%s", $currentPage, max(0, $pageNum_colar - 1), $queryString_colar); ?>"><img src="../imagens/Previous.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_colar < $totalPages_colar) { // Aparece quaso não seja a ultima pagina?>
                    <a href="<?php printf("%s?pageNum_colar=%d%s", $currentPage, min($totalPages_colar, $pageNum_colar + 1), $queryString_colar); ?>"><img src="../imagens/Next.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina?></td>
                <td><?php if ($pageNum_colar < $totalPages_colar) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_colar=%d%s", $currentPage, $totalPages_colar, $queryString_colar); ?>"><img src="../imagens/Last.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
              </tr>
            </table>
            </p>
