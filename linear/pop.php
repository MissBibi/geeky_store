...
$maxRows_pop = 3;
$pageNum_pop = 0;
if (isset($_GET['pageNum_pop'])) {
  $pageNum_pop = $_GET['pageNum_pop'];
}
$startRow_pop = $pageNum_pop * $maxRows_pop;

mysql_select_db($database_geeky_store, $geeky_store);
$query_pop = "SELECT * FROM produto WHERE produto.tipo_codtipo=1 AND produto.status='Disponivel'";
$query_limit_pop = sprintf("%s LIMIT %d, %d", $query_pop, $startRow_pop, $maxRows_pop);
$pop = mysql_query($query_limit_pop, $geeky_store) or die(mysql_error());
$row_pop = mysql_fetch_assoc($pop);

if (isset($_GET['totalRows_pop'])) {
  $totalRows_pop = $_GET['totalRows_pop'];
} else {
  $all_pop = mysql_query($query_pop);
  $totalRows_pop = mysql_num_rows($all_pop);
}
$totalPages_pop = ceil($totalRows_pop/$maxRows_pop)-1;
...
$queryString_pop = sprintf("&totalRows_pop=%d%s", $totalRows_pop, $queryString_pop);
	...
	<!-- Featured -->
      ...
      <table class="darkTable">
        ...
        <tbody>
          <?php do { ?>
          <tr>
            <td><?php echo $row_pop['codproduto']; ?></td>
            <td><?php echo $row_pop['nomep']; ?></td>
            <td><?php echo $row_pop['precou'];?></td>
            <td><?php echo $row_pop['stock']; ?></td>
            <td><?php echo $row_pop['descricao']; ?></td>
            <td><input type="image" name="imageField" id="imageField" src="../imagens/<?php echo $row_pop['imagem']; ?>"width="200" height="160"></td>
            <td><a href="compras.php?codigo=<?php echo $row_pop['codproduto']; ?>"><img src="images/kart.png" width="200" height="160"></a></td>
      </tr>
      </tbody>
<?php } while ($row_pop = mysql_fetch_assoc($pop)); ?>
</table>

			<p>&nbsp;
            <table border="0">
              <tr>
                <td><?php if ($pageNum_pop > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_pop=%d%s", $currentPage, 0, $queryString_pop); ?>"><img src="../imagens/First.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_pop > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_pop=%d%s", $currentPage, max(0, $pageNum_pop - 1), $queryString_pop); ?>"><img src="../imagens/Previous.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_pop < $totalPages_pop) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_pop=%d%s", $currentPage, min($totalPages_pop, $pageNum_pop + 1), $queryString_pop); ?>"><img src="../imagens/Next.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
                <td><?php if ($pageNum_pop < $totalPages_pop) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_pop=%d%s", $currentPage, $totalPages_pop, $queryString_pop); ?>"><img src="../imagens/Last.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
              </tr>
            </table>
            </p>
