...
$maxRows_caneca = 3;
$pageNum_caneca = 0;
if (isset($_GET['pageNum_caneca'])) {
  $pageNum_caneca = $_GET['pageNum_caneca'];
}
$startRow_caneca = $pageNum_caneca * $maxRows_caneca;

mysql_select_db($database_geeky_store, $geeky_store);
$query_caneca = "SELECT * FROM produto WHERE produto.tipo_codtipo=6 AND produto.status='Disponivel'";
$query_limit_caneca = sprintf("%s LIMIT %d, %d", $query_caneca, $startRow_caneca, $maxRows_caneca);
$caneca = mysql_query($query_limit_caneca, $geeky_store) or die(mysql_error());
$row_caneca = mysql_fetch_assoc($caneca);

if (isset($_GET['totalRows_caneca'])) {
  $totalRows_caneca = $_GET['totalRows_caneca'];
} else {
  $all_caneca = mysql_query($query_caneca);
  $totalRows_caneca = mysql_num_rows($all_caneca);
}
$totalPages_caneca = ceil($totalRows_caneca/$maxRows_caneca)-1;
...
if (!empty($_SERVER['QUERY_STRING'])) {
  ...
  foreach ($params as $param) {
    if (stristr($param, "pageNum_caneca") == false && 
        stristr($param, "totalRows_caneca") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_caneca = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_caneca = sprintf("&totalRows_caneca=%d%s", $totalRows_caneca, $queryString_caneca);
...
<html>
	...
	<!-- Featured -->
        ...
        <tbody>
          <?php do { ?>
          <tr>
            <td><?php echo $row_caneca['codproduto']; ?></td>
            <td><?php echo $row_caneca['nomep']; ?></td>
            <td><?php echo $row_caneca['precou']; ?></td>
            <td><?php echo $row_caneca['stock']; ?></td>
            <td><?php echo $row_caneca['descricao']; ?></td>
            <td><input type="image" name="imageField" id="imageField" src="../imagens/<?php echo $row_caneca['imagem']; ?>" width="200" height="160"></td>
            <td><a href="compras.php?codigo=<?php echo $row_caneca['codproduto']; ?>"><img src="images/kart.png" width="200" height="160"></a></td>
          </tr>
          <?php } while ($row_caneca = mysql_fetch_assoc($caneca)); ?>  
        </table>
			<p>&nbsp;
            <table border="0">
              <tr>
                <td><?php if ($pageNum_caneca > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_caneca=%d%s", $currentPage, 0, $queryString_caneca); ?>"><img src="../imagens/First.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_caneca > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_caneca=%d%s", $currentPage, max(0, $pageNum_caneca - 1), $queryString_caneca); ?>"><img src="../imagens/Previous.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina?></td>
                <td><?php if ($pageNum_caneca < $totalPages_caneca) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_caneca=%d%s", $currentPage, min($totalPages_caneca, $pageNum_caneca + 1), $queryString_caneca); ?>"><img src="../imagens/Next.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
                <td><?php if ($pageNum_caneca < $totalPages_caneca) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_caneca=%d%s", $currentPage, $totalPages_caneca, $queryString_caneca); ?>"><img src="../imagens/Last.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
              </tr>
            </table>
            </p>
