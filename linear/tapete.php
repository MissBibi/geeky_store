...
$maxRows_tapete = 3;
$pageNum_tapete = 0;
if (isset($_GET['pageNum_tapete'])) {
  $pageNum_tapete = $_GET['pageNum_tapete'];
}
$startRow_tapete = $pageNum_tapete * $maxRows_tapete;

mysql_select_db($database_geeky_store, $geeky_store);
$query_tapete = "SELECT * FROM produto WHERE produto.tipo_codtipo=5 AND produto.status='Disponivel'";
$query_limit_tapete = sprintf("%s LIMIT %d, %d", $query_tapete, $startRow_tapete, $maxRows_tapete);
$tapete = mysql_query($query_limit_tapete, $geeky_store) or die(mysql_error());
$row_tapete = mysql_fetch_assoc($tapete);

if (isset($_GET['totalRows_tapete'])) {
  $totalRows_tapete = $_GET['totalRows_tapete'];
} else {
  $all_tapete = mysql_query($query_tapete);
  $totalRows_tapete = mysql_num_rows($all_tapete);
}
$totalPages_tapete = ceil($totalRows_tapete/$maxRows_tapete)-1;
...
$queryString_tapete = sprintf("&totalRows_tapete=%d%s", $totalRows_tapete, $queryString_tapete);
...
<html>
	...
	<!-- Featured -->
        ...
        <tbody>
          <?php do { ?>
          <tr>
            <td><?php echo $row_tapete['codproduto']; ?></td>
            <td><?php echo $row_tapete['nomep']; ?></td>
            <td><?php echo $row_tapete['precou']; ?></td>
            <td><?php echo $row_tapete['stock']; ?></td>
            <td><?php echo $row_tapete['descricao']; ?></td>
            <td><input type="image" name="imageField" id="imageField" src="../imagens/<?php echo $row_tapete['imagem']; ?>" width="200" height="160"></td>
            <td><a href="compras.php?codigo=<?php echo $row_tapete['codproduto']; ?>"><img src="images/kart.png" width="200" height="160"></a></td>
          </tr>
          <?php } while ($row_tapete = mysql_fetch_assoc($tapete)); ?>
        </table>
			<p>&nbsp;
            <table border="0">
              <tr>
                <td><?php if ($pageNum_tapete > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_tapete=%d%s", $currentPage, 0, $queryString_tapete); ?>"><img src="../imagens/First.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_tapete > 0) { // Aparece quaso não seja a primeira pagina ?>
                    <a href="<?php printf("%s?pageNum_tapete=%d%s", $currentPage, max(0, $pageNum_tapete - 1), $queryString_tapete); ?>"><img src="../imagens/Previous.gif"></a>
                    <?php } // Aparece quaso não seja a primeira pagina ?></td>
                <td><?php if ($pageNum_tapete < $totalPages_tapete) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_tapete=%d%s", $currentPage, min($totalPages_tapete, $pageNum_tapete + 1), $queryString_tapete); ?>"><img src="../imagens/Next.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
                <td><?php if ($pageNum_tapete < $totalPages_tapete) { // Aparece quaso não seja a ultima pagina ?>
                    <a href="<?php printf("%s?pageNum_tapete=%d%s", $currentPage, $totalPages_tapete, $queryString_tapete); ?>"><img src="../imagens/Last.gif"></a>
                    <?php } // Aparece quaso não seja a ultima pagina ?></td>
              </tr>
            </table>
            </p>
