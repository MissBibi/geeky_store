...
$maxRows_almofada = 3;
$pageNum_almofada = 0;
if (isset($_GET['pageNum_almofada'])) {
  $pageNum_almofada = $_GET['pageNum_almofada'];
}
$startRow_almofada = $pageNum_almofada * $maxRows_almofada;

mysql_select_db($database_geeky_store, $geeky_store);
$query_almofada = "SELECT * FROM produto WHERE produto.tipo_codtipo=4  AND produto.status='Disponivel'";
$query_limit_almofada = sprintf("%s LIMIT %d, %d", $query_almofada, $startRow_almofada, $maxRows_almofada);
$almofada = mysql_query($query_limit_almofada, $geeky_store) or die(mysql_error());
$row_almofada = mysql_fetch_assoc($almofada);

if (isset($_GET['totalRows_almofada'])) {
  $totalRows_almofada = $_GET['totalRows_almofada'];
} else {
  $all_almofada = mysql_query($query_almofada);
  $totalRows_almofada = mysql_num_rows($all_almofada);
}
$totalPages_almofada = ceil($totalRows_almofada/$maxRows_almofada)-1;

$queryString_almofada = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_almofada") == false && 
        stristr($param, "totalRows_almofada") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_almofada = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_almofada = sprintf("&totalRows_almofada=%d%s", $totalRows_almofada, $queryString_almofada);
...
<!-- Main -->
	...
	<!-- Featured -->
        ...
        <table class="darkTable" align="right">
        <thead>
          <tr>
            <th>codproduto</th>
        ...
        <tbody>
          <?php do { ?>
          <tr>
            <td><?php echo $row_almofada['codproduto']; ?></td>
            <td><?php echo $row_almofada['nomep']; ?></td>
            <td><?php echo $row_almofada['precou']; ?></td>
            <td><?php echo $row_almofada['stock']; ?></td>
            <td><?php echo $row_almofada['descricao']; ?></td>
            <td><input type="image" name="imageField" id="imageField" src="../imagens/<?php echo $row_almofada['imagem']; ?>" width="200" height="160"></td>
            <td><a href="compras.php?codigo=<?php echo $row_almofada['codproduto']; ?>"><img src="images/kart.png" width="200" height="160"></a></td>
          </tr>
          <?php } while ($row_almofada = mysql_fetch_assoc($almofada)); ?>         
        </table>
			<p>&nbsp;
            <table border="0">
              <tr>
                <td><?php if ($pageNum_almofada > 0) { // Show if not first page ?>
                    <a href="<?php printf("%s?pageNum_almofada=%d%s", $currentPage, 0, $queryString_almofada); ?>"><img src="../imagens/First.gif"></a>
                    <?php } // Show if not first page ?></td>
                <td><?php if ($pageNum_almofada > 0) { // Show if not first page ?>
                    <a href="<?php printf("%s?pageNum_almofada=%d%s", $currentPage, max(0, $pageNum_almofada - 1), $queryString_almofada); ?>"><img src="../imagens/Previous.gif"></a>
                    <?php } // Show if not first page ?></td>
                <td><?php if ($pageNum_almofada < $totalPages_almofada) { // Show if not last page ?>
                    <a href="<?php printf("%s?pageNum_almofada=%d%s", $currentPage, min($totalPages_almofada, $pageNum_almofada + 1), $queryString_almofada); ?>"><img src="../imagens/Next.gif"></a>
                    <?php } // Show if not last page ?></td>
                <td><?php if ($pageNum_almofada < $totalPages_almofada) { // Show if not last page ?>
                    <a href="<?php printf("%s?pageNum_almofada=%d%s", $currentPage, $totalPages_almofada, $queryString_almofada); ?>"><img src="../imagens/Last.gif"></a>
                    <?php } // Show if not last page ?></td>
              </tr>
            </table>
            </p>
